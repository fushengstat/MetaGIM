...
reorganize_ref<- function(thetas,omes,npara,nmap,group,sample.info){
  nmodel <- length(npara)
  ...
  for(i in 1:K){
    map$alp[[i]] <- map$alp[[i]] + nlam + n.the + 1
    all.alp <- c(all.alp, map$alp[[i]])
    map$bet[[i]] <- map$bet[[i]] + nlam + n.the + nalp + 1
