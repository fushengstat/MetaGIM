...
meta_gim <- function(models,group,sample.info,form,family,data){
  ...
  res0 <- reorganize(thetas,npara,nmap,group,sample.info)
  para = res0$para
  map = res0$map
  K = res0$K
  ...
  lam = para[1]
  daty = data[, outcome0]
  Inew = nCov(npara,nmap,score,daty,lam,n.para,J0,Sig0)
  
  # Vthe = Var_cof(nmap,J0,Inew)
  ### One-step meta-analysis for theta
  # res = meta_analysis(thetas,Vthe)
  # res$thetas = lsttovec(thetas)
  
  
  Vthe = Var_cof_new(nmap,J0,Inew)
  nthetas = newtheta(thetas)
  ### One-step meta-analysis for theta
  # regular estimate of theta
  res = meta_analysis(nthetas,Vthe)
  res1 = meta_ind(nthetas,V.the)
  
  coef1 = res$coefficients
  coef2 = res1$coefficients
  vcov1  = res$vcov
  vcov2  = res1$vcov

  list(coef.0 = coef1, coef.i = coef2, vcov.0 = vcov1, vcov.i = vcov2)
}


meta_gim_rho <- function(models,group,sample.info,form,family,data,cut0){
  ncase = sample.info$ncase
  nctrl = sample.info$nctrl
